...
impl MeshBBHGenerator {
    ...
    // TODO rename this func and others
    // returns new segments buffer
    fn reorder_indices(
        &self,
        triangle_info_buffer: &wgpu::Buffer,
        segments: &wgpu::Buffer,
        ...
        let params = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("reorder indices params"),
            contents: bytemuck::cast_slice(&[SPLIT_CANDIDATE_COUNT]),
            usage: wgpu::BufferUsages::UNIFORM,
        });

        let new_segments = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("reoder indices"),
            size: (segment_count * 2 * 8) as u64,
            usage: wgpu::BufferUsages::STORAGE,
            mapped_at_creation: false,
        });
        ...
        let bind_group: wgpu::BindGroup = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: new_segments.as_entire_binding(),
                },
            ],
        });
